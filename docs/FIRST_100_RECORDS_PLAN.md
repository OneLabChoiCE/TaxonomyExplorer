...
4. **Ratify dictionaries** → the 92 A+B records land in their recorded statuses: ~82 head toward `ACTIVE`, while `OCS`/`HSS`/`BAR`/`ZEE` and ASM `DCK` are born `SUPERCLASS`, `SBR`/`HBR` are `RESERVED`-parked, and `CFS`/`PIP`/`TUB` are `REJECTED` (never activate).
...
- [ ] Each record's activation target is its recorded status, not a blanket `ACTIVE` (§9): the 3 `REJECTED` `SEC` rows never activate; the 5 `SUPERCLASS` and 2 `RESERVED` rows take their terminal statuses.

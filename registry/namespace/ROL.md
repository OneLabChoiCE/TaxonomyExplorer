...
# Namespace record — `ROL:` (Component role)
...
## 1. Identity
- **Identifier:** `ROL:` (namespace prefix)
- **Record type:** Namespace
- **Name:** Component role

## 2. Meaning
- **Contains:** Functional roles of components.
- **Code form:** 3-char alpha, always namespace-qualified (e.g. `ROL:COL`).
...
- **Note:** Orthogonal to shape and family — a component's role is independent of its section geometry (`SEC:`) and its product family (`FAM:`).
...
## 5. Relationships
- **Members:** all `ROL:` role codes — realized as the rows of [dictionaries/rol_codes.csv](../../dictionaries/rol_codes.csv); their per-code founding records populate [registry/dictionary/](../dictionary/) (Category B).
...
- **Verify:** this record restates, without alteration, the `ROL:` row of [dictionaries/namespaces.csv](../../dictionaries/namespaces.csv). A namespace is registered by decision, not derived — there is no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

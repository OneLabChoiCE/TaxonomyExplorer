...
# Dictionary code record — `SEC:ZEE` (Z superclass)
...
## 1. Identity
- **Identifier:** `SEC:ZEE` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** Z superclass
- **Namespace:** [`SEC:`](../namespace/SEC.md) — Section shape.

## 2. Meaning
Restates, without alteration, the `ZEE` row of [dictionaries/sec_codes.csv](../../dictionaries/sec_codes.csv):
- **Definition:** Any Z-family profile; classifier-only rollup.
...
- **Not assignable.** As a rollup, `SEC:ZEE` classifies but is never assigned to an object (N6); its members are the active Z codes `SEC:ZLP` / `SEC:ZUN`.
- **Event history:**
  - *Established `SUPERCLASS`* — 2026-07-11, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: the `ZEE` superclass designation (status `S`) in the §3.2 shape-code table (GSID Standard §3.2). `SEC:ZEE` was never a registry-`ACTIVE` code, so no in-registry `ACTIVE → SUPERCLASS` transition occurs. Seed row in `dictionaries/sec_codes.csv`.
...
- **Rolls up (classifier-only):** `SEC:ZLP`, `SEC:ZUN` (the active Z codes).
- **Predecessor / successor:** none (CSV `successor` empty).
- **Crosswalks / aliases:** none at issuance. *(The `ZED` demo alias is a separate, deferred demo-dictionary governance item — First 100 Records Plan §6.3 — not resolved by this record.)*
...
- **Verify:** exact match to the `ZEE` row of [dictionaries/sec_codes.csv](../../dictionaries/sec_codes.csv) — the `status` column matches at every lineage point (this code is not activation-bound, so no lineage-point caveat applies). A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

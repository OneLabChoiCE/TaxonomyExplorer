...
# Dictionary code record — `CFG:FIN` (Finish system)
...
## 1. Identity
- **Identifier:** `CFG:FIN` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** Finish system
- **Namespace:** [`CFG:`](../namespace/CFG.md) — Configuration group.

## 2. Meaning
Restates, without alteration, the `FIN` row of [dictionaries/cfg_groups.csv](../../dictionaries/cfg_groups.csv) (the configuration-group CSV carries `example_fields`; it has no free-text definition or successor column):
- **Example fields:** system (POWDER|PAINT|PRE_GALV|HOT_DIP_GALV|ZINC_PLATE); color (informative).

A dictionary code is registered by TC decision (Registry Architecture §2); it has no derived component. `CFG:FIN` is a **schema organizer, never an object identifier** — it groups identity-bearing/informative fields that configuration templates draw on; it never names a component, assembly, or geometry. The example fields are illustrative of the group's scope, not a fixed template schema (identity-bearing field sets are fixed by the versioned templates in `templates/`, not here).
...
- **Event history:**
  - *Reserved* — 2026-07-12, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: the `FIN` entry in the Configuration group codes table (Taxonomy Standard §3.6); status `ACTIVE` from the seed row in `dictionaries/cfg_groups.csv`.
...
- **Consumed by:** configuration templates (in `templates/`) that draw `FIN` fields into their identity-bearing/informative field sets.
...
- **Verify:** exact match to the `FIN` row of [dictionaries/cfg_groups.csv](../../dictionaries/cfg_groups.csv) — reading the `status` column at its `since_snapshot` lineage point (§3), so every column matches. A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

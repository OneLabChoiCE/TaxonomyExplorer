...
# Dictionary code record — `ASM:BGD` (Bar grating deck)
...
## 1. Identity
- **Identifier:** `ASM:BGD` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** Bar grating deck
- **Namespace:** [`ASM:`](../namespace/ASM.md) — Assembly type.

## 2. Meaning
Restates, without alteration, the `BGD` row of [dictionaries/asm_codes.csv](../../dictionaries/asm_codes.csv) (the `ASM:` dictionary has `definition`/`successor` columns — no `form_class`/`maps_from`):
- **Definition:** Bearing bar + crossbar grating deck.
- **Successor:** none.
...
- **Event history:**
  - *Reserved* — 2026-07-12, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: the `BGD` active designation (status `A`) in the Assembly type codes table (Taxonomy Standard §3.4); seed row in `dictionaries/asm_codes.csv`.
...
- **Superclass:** member of the [`ASM:DCK`](ASM-DCK.md) deck superclass rollup (classifier-only, N6) — `ASM:DCK` rolls up `ASM:WDK` / `ASM:SDK` / `ASM:PDK` / `ASM:BGD`.
...
- **Verify:** exact match to the `BGD` row of [dictionaries/asm_codes.csv](../../dictionaries/asm_codes.csv) — reading the `status` column at its `since_snapshot` lineage point (§3), so every column matches. A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

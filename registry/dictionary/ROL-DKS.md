...
# Dictionary code record — `ROL:DKS` (Deck support member)
...
## 1. Identity
- **Identifier:** `ROL:DKS` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** Deck support member
- **Namespace:** [`ROL:`](../namespace/ROL.md) — Component role.

## 2. Meaning
Restates, without alteration, the `DKS` row of [dictionaries/rol_codes.csv](../../dictionaries/rol_codes.csv):
- **Definition:** Support channels; hat stiffeners; deck reinforcements.
- **Successor:** none.

A dictionary code is registered by TC decision (Registry Architecture §2); it has no derived component. As a component role, `ROL:DKS` is orthogonal to section shape (`SEC:`) and product family (`FAM:`).
...
- **Event history:**
  - *Reserved* — 2026-07-12, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: the `DKS` active designation (status `A`) in the Component role codes table (Taxonomy Standard §3.3); seed row in `dictionaries/rol_codes.csv`.
...
- **Verify:** exact match to the `DKS` row of [dictionaries/rol_codes.csv](../../dictionaries/rol_codes.csv) — reading the `status` column at its `since_snapshot` lineage point (§3), so every column matches. A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

...
# Dictionary code record — `FAM:SHV` (Steel shelving)
...
## 1. Identity
- **Identifier:** `FAM:SHV` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** Steel shelving
- **Namespace:** [`FAM:`](../namespace/FAM.md) — Product family.

## 2. Meaning
Restates, without alteration, the `SHV` row of [dictionaries/fam_codes.csv](../../dictionaries/fam_codes.csv) (the family CSV carries `domain` and `industry_anchor` in place of a free-text definition):
- **Domain:** `MH` (Material Handling).
- **Industry anchor:** SMA shelving scope.
- **Successor:** none.
...
- **Event history:**
  - *Reserved* — 2026-07-12, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: the `SHV` active designation (status `A`) in the Product family codes table (Taxonomy Standard §3.5); seed row in `dictionaries/fam_codes.csv`.
...
- **Source:** project-authored — Taxonomy Standard §3.5 (Product family codes, status `A`), anchored to SMA shelving scope; committed seed row in [dictionaries/fam_codes.csv](../../dictionaries/fam_codes.csv).
...
- **Verify:** exact match to the `SHV` row of [dictionaries/fam_codes.csv](../../dictionaries/fam_codes.csv) — reading the `status` column at its `since_snapshot` lineage point (§3), so every column matches. A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

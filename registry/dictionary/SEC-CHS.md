...
# Dictionary code record — `SEC:CHS` (Circular hollow)
...
## 1. Identity
- **Identifier:** `SEC:CHS` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** Circular hollow
- **Namespace:** [`SEC:`](../namespace/SEC.md) — Section shape.

## 2. Meaning
Restates, without alteration, the `CHS` row of [dictionaries/sec_codes.csv](../../dictionaries/sec_codes.csv):
- **Definition:** Closed circular hollow including pipe geometry.
- **Form class:** `WELDED`.
- **Maps from:** AISC HSS round and Pipe; EN/AS-NZS CHS; GB/T; JIS. *(Informative record of the shape's origin, not a registered crosswalk — designation crosswalks are separate `DSG-` records, deferred per First 100 Records Plan §7.)*
- **Successor:** none.
...
- **Event history:**
  - *Reserved* — 2026-07-12, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: the `CHS` active designation (status `A`) in the §3.2 shape-code table (GSID Standard §3.2); seed row in `dictionaries/sec_codes.csv`.
...
- **Superclass:** member of the [`SEC:HSS`](SEC-HSS.md) hollow-structural superclass rollup (classifier-only, N6) — `SEC:HSS` rolls up `SEC:RHS` / `SEC:SHS` / `SEC:CHS`.
- **Redirect target for burned strings:** the rejected `SEC:PIP` (CSV `successor=CHS`) and `SEC:TUB` ("use RHS/SHS/CHS") point to `SEC:CHS` for the geometry they lacked — pipe schedules are a designation system over `SEC:CHS` geometry, not a separate shape.
...
- **Verify:** exact match to the `CHS` row of [dictionaries/sec_codes.csv](../../dictionaries/sec_codes.csv) — reading the `status` column at its `since_snapshot` lineage point (§3), so every column matches. A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

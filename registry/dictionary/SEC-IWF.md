...
name: I-shape wide/parallel flange
status: RESERVED
...
# Dictionary code record — `SEC:IWF` (I-shape wide/parallel flange)
...
## 1. Identity
- **Identifier:** `SEC:IWF` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** I-shape wide/parallel flange
- **Namespace:** [`SEC:`](../namespace/SEC.md) — Section shape.

## 2. Meaning
Restates, without alteration, the `IWF` row of [dictionaries/sec_codes.csv](../../dictionaries/sec_codes.csv):
- **Definition:** Filleted I/H with parallel flange faces.
- **Form class:** `HOT_ROLLED_FILLETED`.
- **Maps from:** AISC W/HP/M; EN HE/IPE; UB/UC; GB/T HW/HM/HN; JIS H. *(Informative record of the shape's origin, not a registered crosswalk — designation crosswalks are separate `DSG-` records, deferred per First 100 Records Plan §7.)*
- **Successor:** none.
...
- **Event history:**
  - *Reserved* — 2026-07-12, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: the `IWF` active designation (status `A`) in the §3.2 shape-code table (GSID Standard §3.2); seed row in `dictionaries/sec_codes.csv`.
...
- **Superclass:** none — `SEC:IWF` is a standalone active shape code, not a member of any `SEC:` superclass rollup (`SEC:OCS` / `SEC:HSS` / `SEC:BAR` / `SEC:ZEE`).
...
- **Verify:** exact match to the `IWF` row of [dictionaries/sec_codes.csv](../../dictionaries/sec_codes.csv) — reading the `status` column at its `since_snapshot` lineage point (§3), so every column matches. A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

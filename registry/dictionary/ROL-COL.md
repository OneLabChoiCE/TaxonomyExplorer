...
# Dictionary code record — `ROL:COL` (Column / upright post)
...
## 1. Identity
- **Identifier:** `ROL:COL` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** Column / upright post
- **Namespace:** [`ROL:`](../namespace/ROL.md) — Component role.

## 2. Meaning
Restates, without alteration, the `COL` row of [dictionaries/rol_codes.csv](../../dictionaries/rol_codes.csv):
- **Definition:** Rack uprights; shelving posts; platform columns.
- **Successor:** none.

A dictionary code is registered by TC decision (Registry Architecture §2); it has no derived component. As a component role, `ROL:COL` is orthogonal to section shape (`SEC:`) and product family (`FAM:`).
...
- **Event history:**
  - *Reserved* — 2026-07-11, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: Taxonomy Standard Part 3 (code dictionary); seed row in `dictionaries/rol_codes.csv`.
...
- **Source:** project-authored — Taxonomy Standard Part 3; committed seed row in [dictionaries/rol_codes.csv](../../dictionaries/rol_codes.csv).
...
- **Referenced by:** the worked-example component records ([registry/component/](../component/), Category E, deferred) bind this role via its `ROL-COL.v1` configuration template (in `templates/`).
...
- **Verify:** exact match to the `COL` row of [dictionaries/rol_codes.csv](../../dictionaries/rol_codes.csv) — reading the `status` column at its `since_snapshot` lineage point (§3), so every column matches. A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

...
# Dictionary code record — `ASM:DRW` (Drawer system)
...
## 1. Identity
- **Identifier:** `ASM:DRW` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** Drawer system
- **Namespace:** [`ASM:`](../namespace/ASM.md) — Assembly type.

## 2. Meaning
Restates, without alteration, the `DRW` row of [dictionaries/asm_codes.csv](../../dictionaries/asm_codes.csv) (the `ASM:` dictionary has `definition`/`successor` columns — no `form_class`/`maps_from`):
- **Definition:** Modular drawer cabinets and inserts.
- **Successor:** none.
...
- **Event history:**
  - *Reserved* — 2026-07-12, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: the `DRW` active designation (status `A`) in the Assembly type codes table (Taxonomy Standard §3.4); seed row in `dictionaries/asm_codes.csv`.
...
- **Superclass:** none — `ASM:DRW` is a standalone active assembly type, not a member of any `ASM:` superclass rollup (`ASM:DCK` rolls up only the decking assemblies).
...
- **Verify:** exact match to the `DRW` row of [dictionaries/asm_codes.csv](../../dictionaries/asm_codes.csv) — reading the `status` column at its `since_snapshot` lineage point (§3), so every column matches. A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

...
name: Open C unlipped
status: RESERVED
born_snapshot: SNAP-1.0.0
governed_by: GSID Standard v2.0
authorizing_decision: "Founding seed under the bootstrap clause (GOVERNANCE §2); recorded in docs/ENGINEERING_LOG.md (2026-07-12, dictionary batch 3); adopted ACTIVE by founding Ruling R2 (standard Part 3.1); activation gate per FIRST_100_RECORDS_PLAN §6 item 4, §9"
---

# Dictionary code record — `SEC:OCU` (Open C unlipped)
...
## 1. Identity
- **Identifier:** `SEC:OCU` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** Open C unlipped
- **Namespace:** [`SEC:`](../namespace/SEC.md) — Section shape.

## 2. Meaning
Restates, without alteration, the `OCU` row of [dictionaries/sec_codes.csv](../../dictionaries/sec_codes.csv):
- **Definition:** Folded plain C with uniform thickness and no lips.
- **Form class:** `FOLDED_UNIFORM_T`.
- **Maps from:** AISI plain C track/channel. *(Informative record of the shape's origin, not a registered crosswalk — designation crosswalks are separate `DSG-` records, deferred per First 100 Records Plan §7.)*
- **Successor:** none.
...
- **Event history:**
  - *Reserved* — 2026-07-12, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: founding **Ruling R2** — OCU adopted (GSID Standard §3.1); seed row in `dictionaries/sec_codes.csv`.
...
- **Source:** project-authored — GSID Standard §3.1 (Ruling R2, OCU adopted) and the §3.2 shape-code table (status `A`); committed seed row in [dictionaries/sec_codes.csv](../../dictionaries/sec_codes.csv).
...
- **Verify:** exact match to the `OCU` row of [dictionaries/sec_codes.csv](../../dictionaries/sec_codes.csv) — reading the `status` column at its `since_snapshot` lineage point (§3), so every column matches. A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

...
# Dictionary code record — `FAM:LCK` (Lockers)
...
## 1. Identity
- **Identifier:** `FAM:LCK` (namespace-qualified code)
- **Record type:** Dictionary code
- **Name:** Lockers
- **Namespace:** [`FAM:`](../namespace/FAM.md) — Product family.

## 2. Meaning
Restates, without alteration, the `LCK` row of [dictionaries/fam_codes.csv](../../dictionaries/fam_codes.csv) (the family CSV carries `domain` and `industry_anchor` in place of a free-text definition):
...
- **Event history:**
  - *Reserved* — 2026-07-12, founding seed under the bootstrap clause (GOVERNANCE §2); authorizing decision recorded in [ENGINEERING_LOG](../../docs/ENGINEERING_LOG.md). Basis: the `LCK` active designation (status `A`) in the Product family codes table (Taxonomy Standard §3.5); seed row in `dictionaries/fam_codes.csv`.
...
- **Verify:** exact match to the `LCK` row of [dictionaries/fam_codes.csv](../../dictionaries/fam_codes.csv) — reading the `status` column at its `since_snapshot` lineage point (§3), so every column matches. A dictionary code is registered by decision, not derived — no hash to recompute; auditability is by exact match to the governed CSV row and the citing decision.

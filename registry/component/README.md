...
5. The recorded **steward-seeding decision** authorizing Category E + F (a narrow, bounded reconciliation with the Submission Model's stage-S3 intake rule). **Not yet recorded — the sole remaining blocker.**

With items 1–4 satisfied, item 5 (steward-seeding) is the only remaining Category E blocker. Each record will name the bound template version and its identity-bearing field values, and give the path to **recompute the ConfigurationID** (P9) once item 5 clears.
